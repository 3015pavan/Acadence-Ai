...
def start_agent(_role: str = Depends(require_role(["admin"]))):
    return email_agent.start()


@router.post("/stop", response_model=AgentStatusResponse)
def stop_agent(_role: str = Depends(require_role(["admin"]))):
    return email_agent.stop()


@router.post("/run-now", response_model=AgentRunResponse)
def run_agent_now(_role: str = Depends(require_role(["admin", "operator"]))):
    try:
...
def get_agent_logs(limit: int = Query(default=100, ge=1, le=500), _role: str = Depends(require_role(["admin", "operator"]))):
...
def gmail_disconnect(_role: str = Depends(require_role(["admin"]))):
